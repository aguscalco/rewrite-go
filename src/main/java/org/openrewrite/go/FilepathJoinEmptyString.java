...
public class FilepathJoinEmptyString extends Recipe {

    @Override
    public String getDisplayName() {
        return "Use filepath.Clean over filepath.Join with empty string";
    }

    @Override
    public String getDescription() {
        return "Migrates `filepath.Join(path, \"\")` to `filepath.Clean(path)`.";
    }
        ...
        return new GoVisitor<ExecutionContext>() {
            ...
            public Tree visitCallExpr(CallExpr callExpr, ExecutionContext ctx) {
                ...
                if (c.getFun() instanceof SelectorExpr && c.getArgs() != null && c.getArgs().size() == 2) {
                    ...
                    if (sel.getX() instanceof Ident && "filepath".equals(((Ident) sel.getX()).getName()) && "Join".equals(sel.getSel().getName())) {
