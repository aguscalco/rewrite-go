...
public class StringsSplitNToSplit extends Recipe {

    @Override
    public String getDisplayName() {
        return "Use strings.Split instead of strings.SplitN with n=-1";
    }

    @Override
    public String getDescription() {
        return "Migrates `strings.SplitN(s, sep, -1)` to `strings.Split(s, sep)`.";
    }
        ...
        return new GoVisitor<ExecutionContext>() {
            ...
            public Tree visitCallExpr(CallExpr callExpr, ExecutionContext ctx) {
                ...
                if (c.getFun() instanceof SelectorExpr && c.getArgs() != null && c.getArgs().size() == 3) {
                    ...
                    if (sel.getX() instanceof Ident && "strings".equals(((Ident) sel.getX()).getName()) && "SplitN".equals(sel.getSel().getName())) {
