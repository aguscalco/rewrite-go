...
public class SortInts extends Recipe {

    @Override
    public String getDisplayName() {
        return "Use sort.Ints";
    }

    @Override
    public String getDescription() {
        return "Migrates `sort.Sort(sort.IntSlice(x))` to `sort.Ints(x)`.";
    }
                        ...
                        if (arg instanceof CallExpr) {
                            ...
                            if (argCall.getFun() instanceof SelectorExpr) {
                                ...
                                if (argSel.getX() instanceof Ident && "sort".equals(((Ident) argSel.getX()).getName()) && "IntSlice".equals(argSel.getSel().getName())) {
                                    ...
                                    Ident intsIdent = sel.getSel().withName("Ints");
                                    return c.withFun(sel.withSel(intsIdent)).withArgs(argCall.getArgs());
                                }
                            }
                        }
                    }
