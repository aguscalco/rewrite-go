...
public class StringsCompareToEqual extends Recipe {

    @Override
    public String getDisplayName() {
        return "Use == instead of strings.Compare";
    }

    @Override
    public String getDescription() {
        return "Replaces `strings.Compare(a, b) == 0` with `a == b` for better readability, matching staticcheck's SA1019 or gocritic.";
    }
                ...
                if ("==".equals(b.getOp())) {
                    if (b.getX() instanceof CallExpr && b.getY() instanceof BasicLit) {
                        ...
                        if ("0".equals(lit.getValue()) && isStringsCompare(call)) {
                            return new BinaryExpr(b.getId(), b.getPrefix(), b.getMarkers(), call.getArgs().get(0).withPrefix(Space.EMPTY), "==", call.getArgs().get(1).withPrefix(Space.build(" ")), b.getType());
                        }
