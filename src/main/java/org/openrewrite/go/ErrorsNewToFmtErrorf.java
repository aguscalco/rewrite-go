...
public class ErrorsNewToFmtErrorf extends Recipe {
    ...
    public String getDescription() {
        return "Migrates `errors.New(fmt.Sprintf(\"...\", args))` to `fmt.Errorf(\"...\", args)`.";
    }
                        ...
                        if (arg instanceof CallExpr) {
                            ...
                            if (argCall.getFun() instanceof SelectorExpr) {
                                ...
                                if (argSel.getX() instanceof Ident && "fmt".equals(((Ident) argSel.getX()).getName()) && "Sprintf".equals(argSel.getSel().getName())) {
                                    ...
                                    Ident errorfIdent = argSel.getSel().withName("Errorf");
                                    SelectorExpr newSel = argSel.withSel(errorfIdent).withPrefix(sel.getPrefix());
                                    
                                    return c.withFun(newSel).withArgs(argCall.getArgs());
                                }
                            }
                        }
                    }
