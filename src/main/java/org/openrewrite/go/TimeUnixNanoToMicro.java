...
public class TimeUnixNanoToMicro extends Recipe {

    @Override
    public String getDisplayName() {
        return "Use time.UnixMicro";
    }

    @Override
    public String getDescription() {
        return "Migrates `time.Now().UnixNano() / 1000` to `time.Now().UnixMicro()` (Go 1.17+).";
    }
            ...
            public Tree visitBinaryExpr(BinaryExpr binaryExpr, ExecutionContext ctx) {
                ...
                if ("/".equals(b.getOp())) {
                    if (b.getX() instanceof CallExpr && b.getY() instanceof BasicLit) {
                        ...
                        if ("1000".equals(lit.getValue()) || "1e3".equals(lit.getValue())) {
                            if (isUnixNano(call)) {
                                SelectorExpr sel = (SelectorExpr) call.getFun();
                                Ident microIdent = sel.getSel().withName("UnixMicro");
                                return call.withFun(sel.withSel(microIdent)).withPrefix(b.getPrefix());
                            }
                        }
                    }
                }
